...
class Attack(Submission):
  ...
  def run(self, input_dir, output_dir, epsilon):
    ...
    cmd = ['./' + self.entry_point,
           ...
           str(epsilon)]
    print(' '.join(cmd))
    subprocess.call(cmd, cwd='{0}'.format(self.directory))
...
class AttacksOutput(object):
  ...
  def clip_and_copy_attack_outputs(self, attack_name, is_targeted):
    ...
    for fname in os.listdir(attack_dir):
      ...
      fileName = fname.split(r'.')[0]
      defenseID = fileName.split(r'_')[-1] #orig_i, i means defense id
      Image.fromarray(clipped_adv_image).save(
          os.path.join(self.all_adv_examples_dir, output_basename + '_' + attack_name + '_' + defenseID + '.png'))
...
def main():
  ...
  """
  attacks_output_dir = r'NOT_supported'
