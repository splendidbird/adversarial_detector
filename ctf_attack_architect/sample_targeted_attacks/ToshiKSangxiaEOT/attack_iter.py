...
tf.flags.DEFINE_string(
     'output_attack_dir', r'../../../adv_images/', 'Output directory with images for attack')
...
tf.flags.DEFINE_integer(
    'ensemble_size', 10, 'how many times to tile the input tensor')
...
def load_images(input_dir, batch_shape):
    ...
    for filepath in tf.gfile.Glob(os.path.join(input_dir, '*.png')):
        ...
        if int(defenseID) not in _targetAttackChosenDiction['ToshiKSangxiaEOT']:
            continue
...
def save_images(images, filenames, output_dir):
    ...
    for i, filename in enumerate(filenames):
        ...
        filename = 'ToshiSXEOT'+ get_file_config() + filename + r'.png'
...
def addNoise(x, batch_shape):
    #print('log: adding noise')
    x_adv = x + 0.1 * tf.sign(tf.random_normal(batch_shape))
    return x_adv

def defend_noise(x_expanded, ensemble_size, batch_shape):
    ensemble_xs = tf.concat([addNoise(x_expanded, batch_shape) for _ in range(ensemble_size)], axis=0)
    return ensemble_xs
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        ensemble_xs = defend_noise(x_expanded, ensemble_size = EOT, batch_shape = batch_shape)
        ...
        ensemble_logits = ensemble_logits_cs1 + ensemble_logits_cs2 + ensemble_logits_cs3 + 0.4 * (ensemble_end_points_cs1['AuxLogits'] + ensemble_end_points_cs2['AuxLogits'] + ensemble_end_points_cs3['AuxLogits'])

        ensemble_logits = ensemble_logits / 3
        if DEBUG:
            ensemble_probs = tf.reduce_mean(tf.nn.softmax(ensemble_logits), axis=0, keep_dims=True)
            ensemble_preds = tf.argmax(ensemble_probs, 1)
        ...
        ensemble_xent = tf.reduce_mean(tf.losses.softmax_cross_entropy(logits=ensemble_logits, onehot_labels=ensemble_labels))

        lam = tf.placeholder(tf.float32, ())
        ensemble_loss = ensemble_xent + lam * normalized_l2_loss

        ensemble_grad, = tf.gradients(ensemble_loss, x_expanded)
        """
        ...
        with tf.train.MonitoredSession(session_creator=session_creator) as sess:
            ...
            for filenames, images in load_images(FLAGS.input_dir, batch_shape):
                ...
                for i in range(ITER_NUM):
                    if DEBUG:
                        g, l2, p = sess.run([ensemble_grad, normalized_l2_loss, ensemble_preds], {x_expanded: adv, target_class_input: target_class_for_batch, lam: 1.0, l2_x: adv, l2_orig: images})
                        ...
                        g = sess.run([ensemble_grad], {x_expanded: adv, target_class_input: target_class_for_batch, lam: 1.0, l2_x: adv, l2_orig: images})
                    adv -= LR * g
                    # project
