...
tf.flags.DEFINE_integer(
    'EOT', 15, 'Number of iterations for random_normal.')
...
def save_images(images, filenames, output_dir):
  ...
  for i, filename in enumerate(filenames):
    ...
    filename = 'SangxiaToshiK'+ get_file_config() + filename + r'.png'
...
def main(_):
  ...
  with tf.Graph().as_default():
      ...
      for _ in range(ITER_NUM): # no. of iteration. tested 1,2,4,15
	  #Sangxia
          x_adv = x_adv + 0.1*alpha*tf.sign(tf.random_normal(batch_shape))

	  # remove seed for actual random number
	  # x_adv = x_adv + 0.1*alpha*tf.sign(tf.random_normal((16,299,299,3)))
	  #end
	  ...
	  # Gradient smoothing. Comment below to remove gradient smoothing
          grad = tf.nn.depthwise_conv2d(grad, stack_kernel, strides=[1, 1, 1, 1], padding='SAME')

 	  # alpha skew
          x_next = x_adv - 4.0*alpha * tf.sign(grad)
