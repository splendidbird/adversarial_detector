...
class FileEventHandler(FileSystemEventHandler):
    def __init__(self, batch_size, input_dir, net1, net4, model, itr, output_dir, no_gpu, kmean):
    ...
    def _defense_for_img_created(self, imgfile):
        ...
        for filename, label in final_labels.items():
            ...
            # final check with kmean clustering
            kmean_img = os.path.join(self._input_dir, os.path.basename(filename))
            kmean_label = self._kmean.compare(kmean_img,label)

            #print(output_file_name)
            #print("res_file_name: " + res_file_name)
            with open(os.path.join(self._output_dir, res_file_name), 'w+') as res_file:
                res_file.write('{0}\n{1}\n'.format(kmean_label,
                                                   self._category_helper.get_category_name(kmean_label)))
                res_file.flush()
